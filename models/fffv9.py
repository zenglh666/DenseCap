...
def transformer_decoder(inputs, memory, bias, src_mask, params, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        for layer in range(params.num_decoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                ...
                language = x * tf.expand_dims(src_mask, -1)
...
def encoding_graph_bert(src_seq, src_len, mode, params):
    ...
    return encoder_output, enc_attn_bias, src_mask
...
def model_graph(features, mode, params):
    ...
    with tf.variable_scope("proposal"):
        ...
        with tf.variable_scope("word_embedding"):
            if params.use_bert:
                caption_feature_base, enc_attn_bias, src_mask = encoding_graph_bert(label, label_length, mode, params)
            else:
                caption_feature_base, enc_attn_bias = encoding_graph(label, label_length, mode, params)

        if params.task == "didemo":
            #feature = tf.concat([feature, tf.tile(tf.reduce_mean(feature, axis=1, keepdims=True), [1, tf.shape(feature)[1], 1])], axis=-1)
            duration = tf.fill(tf.shape(duration), 30.)
        ...
        for layer_id in range(params.anchor_layers):
            with tf.variable_scope("query", reuse=False if layer_id==0 else True):
                ...
                if params.skip_word:
                    ...
                    x = tf.tile(x, [1, tf.shape(caption_feature_base)[1], 1])
                    x = tf.concat([x, caption_feature_base], axis=-1)
                    cell_enc = []
                    cell_enc.append(tf.nn.rnn_cell.BasicLSTMCell(params.hidden_size))
                    cell_enc.append(tf.nn.rnn_cell.BasicLSTMCell(params.hidden_size))
                    cell_enc = tf.nn.rnn_cell.MultiRNNCell(cell_enc)

                    with tf.variable_scope("encoder"):
                        output, final_state = tf.nn.dynamic_rnn(cell_enc, x, dtype=tf.float32)
                else:
                    x = transformer_decoder(x, caption_feature_base, enc_attn_bias, src_mask, params)

                outputs = output[:, -1:, :]
                outputs_list.append(outputs)
    
    if params.proposalv2:
        back_event, proposal = get_proposalv2(params, outputs_list, outputs_deconv_list, outputs_foconv_list)
    else:
    ...
    if params.hard_regress:
        timestamps = timestamps * duration_exp
        proposal = proposal * duration_exp
        params.eucloss_ratio = 1.
